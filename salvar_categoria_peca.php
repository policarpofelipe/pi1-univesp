...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: listar_categorias_peca.php?erro=metodo_invalido');
    exit;
}
...
if ($nome === '') {
    header('Location: form_categoria_peca.php?erro=nome_obrigatorio' . ($id > 0 ? '&id=' . $id : ''));
    exit;
}

if (mb_strlen($nome) > 100) {
    header('Location: form_categoria_peca.php?erro=nome_maior_que_limite' . ($id > 0 ? '&id=' . $id : ''));
    exit;
}
...
try {
    if ($id > 0) {
        ...
        if ($stmtDuplicidade->fetch()) {
            header('Location: form_categoria_peca.php?id=' . $id . '&erro=nome_duplicado');
            exit;
        }
        ...
        header('Location: listar_categorias_peca.php?sucesso=editado');
        exit;
    }
    ...
    if ($stmtDuplicidade->fetch()) {
        header('Location: form_categoria_peca.php?erro=nome_duplicado');
        exit;
    }

    $sqlInsert = "
        INSERT INTO categorias_peca (
            nome,
            descricao,
            ativo,
            criado_em,
    ...
    header('Location: listar_categorias_peca.php?sucesso=cadastrado');
    exit;

} catch (Throwable $e) {
    header('Location: form_categoria_peca.php?erro=erro_interno' . ($id > 0 ? '&id=' . $id : ''));
    exit;
}