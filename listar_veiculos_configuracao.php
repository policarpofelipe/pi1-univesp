...
$sql .= " ORDER BY mv.nome ASC, mo.nome ASC, vc.ano_inicio ASC, vc.versao ASC";
    ...
    <main class="flex-1 p-4 md:p-6 pb-24 md:pb-6">
        <div class="mx-auto max-w-7xl">
            ...
            <div class="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900">Configurações Veiculares</h1>
                    <p class="mt-1 text-sm text-slate-600">
                        Gerencie ano, motorização, combustível e versão de cada modelo.
                    </p>
                </div>
            ...
            <div class="<?= classe_box() ?> mb-6">
                <form method="GET" class="grid grid-cols-1 gap-4 md:grid-cols-12 md:items-end">
                    ...
                    <div class="md:col-span-3 flex gap-2">
                        ...
                        <?= botao_link('listar_veiculos_configuracao.php', 'Limpar', 'cancelar', ['style' => 'width:100%; text-align:center;']) ?>
                    </div>
                </form>
            </div>
            ...
            <div class="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <div class="<?= classe_box() ?>">
                    ...
                    <div class="mt-2 text-3xl font-bold text-slate-900"><?= $totalConfiguracoes ?></div>
                </div>
                        ...
                        <table class="min-w-full border-separate border-spacing-y-2">
                            ...
                            <tbody>
                                <?php foreach ($configuracoes as $config): ?>
                                    <?php $ativo = (int)($config['ativo'] ?? 0) === 1; ?>
                                    <tr class="overflow-hidden rounded-xl bg-slate-50 shadow-sm">
                                        ...
                                        <td class="px-4 py-4">
                                            <span class="inline-flex rounded-full bg-slate-200 px-3 py-1 text-xs font-medium text-slate-700">
                                                <?= esc($config['marca_nome']) ?>
                                            </span>
                                        </td>

                                        <td class="px-4 py-4">
                                            <div class="font-semibold text-slate-900">
                                                <?= esc($config['modelo_nome']) ?>
                                            </div>
                                        </td>

                                        <td class="px-4 py-4 text-sm text-slate-700">
                                            <?php if ((int)$config['ano_inicio'] === (int)$config['ano_fim']): ?>
                                                <?= (int)$config['ano_inicio'] ?>
                                            <?php else: ?>
                                                <?= (int)$config['ano_inicio'] ?> a <?= (int)$config['ano_fim'] ?>
                                            <?php endif; ?>
                                        </td>
                                        ...
                                        <td class="rounded-r-xl px-4 py-4">
                                            <div class="flex items-center justify-end gap-2">
                                                <?= botao_link(
                                                    'form_veiculo_configuracao.php?id=' . (int)$config['id'],
                                                    'Editar',
                                                    'editar'
                                                ) ?>
