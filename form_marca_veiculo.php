...
$modoEdicao = $id > 0;
$tituloPagina = $modoEdicao ? 'Editar Marca de Veículo' : 'Nova Marca de Veículo';

$erroCodigo = trim((string)($_GET['erro'] ?? ''));
$erro = '';

$mapaErros = [
    'nome_obrigatorio'        => 'Informe o nome da marca do veículo.',
    'nome_maior_que_limite'   => 'O nome da marca excede o limite permitido.',
    'nome_duplicado'          => 'Já existe uma marca de veículo cadastrada com este nome.',
    'registro_nao_encontrado' => 'Marca de veículo não encontrada.',
...
if ($modoEdicao) {
    ...
    if (!$marca) {
        $erro = 'Marca de veículo não encontrada.';
        $modoEdicao = false;
        $tituloPagina = 'Nova Marca de Veículo';
    } else {
        $dados = [
            'id'    => (int)$marca['id'],
    ...
    <main class="flex-1 p-4 md:p-6 pb-24 md:pb-6">
        <div class="mx-auto max-w-4xl">
            ...
            <div class="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-600">
                        <?= $modoEdicao
                            ? 'Atualize os dados da marca veicular.'
                            : 'Cadastre uma nova marca para o catálogo de veículos.' ?>
                    </p>
                </div>

                <div class="flex flex-wrap gap-2">
                    <?= botao_link('listar_marcas_veiculo.php', 'Voltar para listagem', 'cancelar') ?>
                </div>
            </div>
            ...
            <div class="<?= classe_box() ?>">
                <form action="salvar_marca_veiculo.php" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            ...
                            <?= input_texto('nome', $dados['nome'], [
                                ...
                                'maxlength' => '100',
                                'required' => true,
                                'placeholder' => 'Ex.: Chevrolet, Fiat, Volkswagen, Ford'
                            ]) ?>
                            <p class="mt-1 text-xs text-slate-500">
                                Informe a fabricante ou marca principal do veículo.
                            </p>
                        </div>
                    ...
                    <div class="flex flex-col gap-2 border-t border-slate-200 pt-4 sm:flex-row">
                        ...
                        <?= botao_link('listar_marcas_veiculo.php', 'Cancelar', 'cancelar') ?>
                    </div>
                </form>
            </div>
            ...
            <div class="mt-6 rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-600 shadow-sm">
                ...
                marca de veículo pertence ao catálogo veicular e não deve ser confundida com
                <em>marca de produto</em>, que representa a marca comercial da peça vendida.
            </div>
        </div>
    </main>
</div>
